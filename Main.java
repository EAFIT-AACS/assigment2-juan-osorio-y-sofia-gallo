...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 5; i++) {
            validStrings[i] = StringGenerator.generateValidString();
            System.out.println((i+1) + ". '" + validStrings[i] + "'");
        }
        System.out.println("\nCadenas inválidas generadas:");
        for (int i = 0; i < 5; i++) {
            invalidStrings[i] = StringGenerator.generateInvalidString();
        ...
        for (int i = 0; i < validStrings.length; i++) {
            ...
            boolean accepted = PDA.accepts(validStrings[i], steps);
        ...
        for (int i = 0; i < invalidStrings.length; i++) {
            List<String> steps = new ArrayList<>();
            boolean accepted = PDA.accepts(invalidStrings[i], steps);
        ...
        for (int i = 0; i < validStrings.length; i++) {
            ...
            if (accepted) {
                ...
                List<String> derivation = DerivationTree.generateLeftmostDerivation(validStrings[i]);
                for (int j = 0; j < derivation.size(); j++) {
                    System.out.println((j+1) + ". " + derivation.get(j));
                }

                System.out.println("\nÁrbol de derivación:");
                DerivationTree.drawDerivationTree(validStrings[i]);
            }
        }
    }
}